...
class PlayScreen extends Component {
  ...
  componentDidUpdate() {
    ...
    if (!game.choices && !error) {
      ...
      if (
        game.result.length < matrivSettings.numberOfQuestions &&
  ...
  componentWillUnmount() {
    ...
    if (history.location.pathname !== Routes.Results) {
      // clear all matriv state (set defaults)
      clearGame();
    }
  }
...
const mapStateToProps = state => ({
  ...
  game: state.matriv.game,
  isGameLoading: state.matriv.fetching,
  error: state.matriv.error,
  isHintOpen: state.ui.isHintOpen,
  attributionText: state.matriv.attributionText
});
